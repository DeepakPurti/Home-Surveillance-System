...
%                 personframe = read(vidObj,42)
%                 imwrite(personframe,'persondetected.jpg');
%                 flag=1;
%             end
            ...
            if(flag<=20)
                imwrite(frame,'persondetected.jpg');
                if(flag==20)
                   sgtitle(sprintf('PERSON DETECTED'),'FontSize',30);
                   persondetectmail();
                   pause(4);
                   sgtitle('');
                end
    ...
    if fallDetected == true   
        ...
        imshow(filledCannyMask);
        title(sprintf('Shape of body\nOrientation - %f',orientation),'FontSize',20);
        sendfallemail(); %send email alert about fall detection
        pause(4);
        sgtitle('');
